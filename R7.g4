...
expression returns [String type]: l1=lsum {$type = $l1.type;};
lsum returns [String type]: lmult s1=('||'|'|') expression {$type="boolean"} | l1=lmult {$type = $l1.type;};
lmult returns [String type]: lnot s1=('&&'|'&') lmult {$type = "boolean";} | l1=lnot {$type = $l1.type;};
lnot returns [String type]: '!'lnot {$type = "boolean";}| c1=comp {$type = $c1.type;};
comp returns [String type]: sum s1=('>'|'>='|'<'|'<='|'=='|'!=') comp {$type = "boolean";} | s2=sum {$type = $s2.type;};
sum returns [String type]: mult s1=('+'|'-') expression {$type = "number";}| m1=mult {$type = $m1.type;};
mult returns [String type]: pow s1=('*'|'/'|'%%'|'%/%') mult {$type = "number";}| p1=pow {$type = $p1.type;};
pow returns [String type]: par s1=('**'|'^') pow {$type = "number";} | p1=par {$type = $p1.type;};
par returns [String type]:s1='('(e1=expression)s2=')' {$type = $e1.type;}|(c1=constant{$type = $c1.type;}|symbol{$type = "symbol";});
